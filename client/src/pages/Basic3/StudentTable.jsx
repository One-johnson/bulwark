...
const StudentTable = ({ filters, searchText }) => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3002/basic3/")
  ...
  const confirmDelete = () => {
    axios
      .delete("http://localhost:3002/basic3/delete/" + confirmDeleteId)
      .then(() => {
  ...
  const handleRowClick = (row) => {
    navigate(`/basic3/view/${row.customID}`);
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="flex space-x-3 items-center text-lg">
          <Link to={`/basic3/view/${row.customID}`} className="text-blue-600">
            <FaRegEye />
          </Link>
          <Link to={`/basic3/edit/${row.customID}`} className="text-green-600">
            <FaRegEdit />
          </Link>
          <button
