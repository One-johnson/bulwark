...
const StudentTable = ({ filters, searchText }) => {
  ...
  const confirmDelete = () => {
    axios
      .delete("http://localhost:3002/kg2/delete/" + confirmDeleteId)
      .then(() => {
  ...
  const handleRowClick = (row) => {
    navigate(`/kg2/view/${row.customID}`);
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="flex space-x-3 items-center text-lg">
          <Link to={`/kg2/view/${row.customID}`} className="text-blue-600">
            <FaRegEye />
          </Link>
          <Link
            to={`/kg2/edit/${row.customID}`}
